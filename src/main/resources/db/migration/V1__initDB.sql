...
CREATE TYPE tipo_evento AS ENUM ('team', 'personale');

CREATE TABLE evento(id_evento UUID PRIMARY KEY, id_organizzazione UUID NOT NULL REFERENCES organizzazione (id_organizzazione), id_utente UUID REFERENCES utente (id_utente), tipo tipo_evento NOT NULL, titolo VARCHAR(50), descrizione VARCHAR(255), time_inizio TIMESTAMP WITH TIME ZONE NOT NULL, time_fine TIMESTAMP WITH TIME ZONE NOT NULL, time_notifica TIMESTAMP WITH TIME ZONE NOT NULL, notificato BOOLEAN DEFAULT false, CHECK(tipo='team' OR id_utente IS NOT NULL));
